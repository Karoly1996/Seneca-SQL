...
int main(void)
{
 ...
 /* Used Variables */
 string str;
 string user = "dbs311_241nee17";
 string pass = "15849173";
...
void displayOrderStatus(Connection* conn, int orderId, int customerId) {
 ...
 stmt->registerOutParam(2, Type::OCCISTRING, sizeof(string)); // Register the output parameter for status
 stmt->executeUpdate(); // Execute the procedure
 string orderStatus = stmt->getString(2); // Retrieve the order status
 conn->terminateStatement(stmt); // Terminate the statement
 if (orderStatus.empty()) {
 cout << "Order does not exist." << endl;
 }
 else {
 cout << "Order status is: " << orderStatus << "." << endl;
 }
 }
 catch (SQLException& sqlExcp) {
...
void cancelOrder(Connection* conn, int orderId, int customerId) {
 try {
 // Check if customer_order procedure exists and matches
 // PROCEDURE customer_order(customerId IN NUMBER, orderId IN NUMBER, validOrderId OUT NUMBER);
 // If so, we should call it before attempting to cancel the order.
 // Prepare to call the cancel_order procedure
 Statement* stmt = conn->createStatement();
